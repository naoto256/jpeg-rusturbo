...
pub mod encode {
    ...
    pub fn quantize_mcu_422_rgb_full(
        ...
        let mut cb_blk = [0i16; 64];
        let mut cr_blk = [0i16; 64];
        crate::color::extract_mcu_422_rgb_full(
            pixels,
            width,
            x0,
            y0,
        ...
        crate::arch::backend::dct::fdct_islow(&mut cb_blk);
        out[2] = quant::quantize_and_zigzag(&cb_blk, div_chroma);
        crate::arch::backend::dct::fdct_islow(&mut cr_blk);
        out[3] = quant::quantize_and_zigzag(&cr_blk, div_chroma);
    }
}
...
pub mod huffman {
    ...
    pub fn ac_magnitudes(block: &[i16; 64], sizes: &mut [u8; 64], bits_lut: &mut [u16; 64]) {
        for (k, &v) in block.iter().enumerate() {
            let (sz, b) = crate::encode::huffman::magnitude_category(v as i32);
            sizes[k] = sz;
            bits_lut[k] = b as u16;
        }
    }
